...
CustomLcdDisplay::CustomLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    native_snapshot_buf = (uint8_t *)heap_caps_malloc(lcd_spi_data.buffer_len, MALLOC_CAP_SPIRAM);
    assert(native_snapshot_buf);
    memset(native_snapshot_buf, 0xFF, lcd_spi_data.buffer_len);
...
void CustomLcdDisplay::SaveNativeSnapshot() {
    if (buffer == nullptr || native_snapshot_buf == nullptr || dirty_mutex == nullptr) {
        return;
    }

    xSemaphoreTake(dirty_mutex, portMAX_DELAY);
    memcpy(native_snapshot_buf, buffer, lcd_spi_data.buffer_len);
    native_snapshot_valid = true;
    xSemaphoreGive(dirty_mutex);
}

void CustomLcdDisplay::RestoreNativeSnapshot() {
    if (buffer == nullptr || native_snapshot_buf == nullptr || dirty_mutex == nullptr || !native_snapshot_valid) {
        return;
    }

    xSemaphoreTake(dirty_mutex, portMAX_DELAY);
    memcpy(buffer, native_snapshot_buf, lcd_spi_data.buffer_len);
    ...
    force_full_refresh_ = true;
    UpdateDisplayBusyLocked();
    sm_kick(kDisplayKickMs, "display_native_restore");
    if (refresh_task) {
        xTaskNotifyGive(refresh_task);
    }
    xSemaphoreGive(dirty_mutex);
