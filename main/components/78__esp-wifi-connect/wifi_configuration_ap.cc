...
bool IsValidMeetingUrl(const std::string& url)
{
    if (url.empty()) {
        return true;
    }
...
WifiConfigurationAp::WifiConfigurationAp()
{
    ...
    meeting_url_ = CONFIG_MEETING_ASSISTANT_URL;
}
...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    if (err == ESP_OK) {
        // 读取会议助手服务地址
        char meeting_url[256] = {0};
        size_t meeting_url_size = sizeof(meeting_url);
        err = nvs_get_str(nvs, kMeetingUrlKey, meeting_url, &meeting_url_size);
        if (err == ESP_OK && IsValidMeetingUrl(meeting_url)) {
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            cJSON *meeting_url_item = cJSON_GetObjectItemCaseSensitive(json, "meeting_url");
            ...
            std::string meeting_url_str;
            if (cJSON_IsString(meeting_url_item) && meeting_url_item->valuestring != NULL) {
                meeting_url_str = meeting_url_item->valuestring;
                if (!IsValidMeetingUrl(meeting_url_str)) {
                    cJSON_Delete(json);
                    httpd_resp_send(req, "{\"success\":false,\"error\":\"Invalid meeting URL\"}", HTTPD_RESP_USE_STRLEN);
                    return ESP_OK;
                }
            }
            ...
            if (cJSON_IsString(meeting_url_item) && meeting_url_item->valuestring != NULL) {
                std::string error;
                if (!this_->SaveMeetingUrl(meeting_url_str, &error)) {
                    cJSON_Delete(json);
                    std::string response = "{\"success\":false,\"error\":\"" + error + "\"}";
    ...
    httpd_uri_t advanced_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            cJSON_AddStringToObject(json, "meeting_url", this_->meeting_url_.c_str());
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // 保存会议助手服务地址
            cJSON *meeting_url = cJSON_GetObjectItem(json, "meeting_url");
            if (cJSON_IsString(meeting_url) && meeting_url->valuestring) {
                std::string meeting_url_str = meeting_url->valuestring;
                if (!IsValidMeetingUrl(meeting_url_str)) {
                    nvs_close(nvs);
                    cJSON_Delete(json);
                    httpd_resp_send(req, "{\"success\":false,\"error\":\"Invalid meeting URL\"}", HTTPD_RESP_USE_STRLEN);
                ...
                if (meeting_url_str.empty()) {
                    this_->meeting_url_ = CONFIG_MEETING_ASSISTANT_URL;
                    err = nvs_erase_key(nvs, kMeetingUrlKey);
                    if (err == ESP_ERR_NVS_NOT_FOUND) {
                        err = ESP_OK;
                    }
                } else {
                    this_->meeting_url_ = meeting_url_str;
                    err = nvs_set_str(nvs, kMeetingUrlKey, this_->meeting_url_.c_str());
                }
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to save meeting URL: %d", err);
                }
            }
            ...
            ESP_LOGI(TAG, "Saved settings: ota_url=%s, meeting_url=%s, max_tx_power=%d, remember_bssid=%d, sleep_mode=%d",
                this_->ota_url_.c_str(), this_->meeting_url_.c_str(), this_->max_tx_power_, this_->remember_bssid_, this_->sleep_mode_);
            return ESP_OK;
        },
        .user_ctx = this
    };
...
bool WifiConfigurationAp::SaveMeetingUrl(const std::string& meeting_url, std::string* error)
{
    ...
    esp_err_t err = nvs_open("wifi", NVS_READWRITE, &nvs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open NVS for meeting URL: %d", err);
        if (error != nullptr) {
    ...
    if (meeting_url.empty()) {
        ...
        meeting_url_ = meeting_url;
        err = nvs_set_str(nvs, kMeetingUrlKey, meeting_url_.c_str());
    }
    ...
    ESP_LOGI(TAG, "Saved meeting URL: %s", meeting_url_.c_str());
    return true;
}
